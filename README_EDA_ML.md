...
1. 하이퍼파라미터 튜닝의 효과
- GridSearchCV를 활용한 체계적 파라미터 탐색으로 최고 성능(정확도 0.8389, F1 0.8761) 달성
- HyperOpt 역시 우수한 성능(정확도 0.8365, F1 0.8777)으로, 하이퍼파라미터 최적화의 중요성 확인
- 기본 모델 대비 정확도 1.68%p, F1 점수 3.87%p 향상
...
> Random Forest의 5가지 접근법을 체계적으로 비교 분석한 결과, **GridSearchCV를 통한 하이퍼파라미터 튜닝이 가장 우수한 성능**을 보였습니다. 이는 해당 데이터셋의 특성상 최적 파라미터 조합이 존재하며, 체계적인 파라미터 탐색이 성능 향상에 기여함을 의미합니다. 기본 모델 대비 정확도 1.68%p, F1 점수 3.87%p 향상으로 **하이퍼파라미터 튜닝의 효과성**을 확인할 수 있었습니다.
